...
import { format, subDays, isAfter, parseISO, startOfWeek, endOfWeek, startOfMonth, endOfMonth, isSameWeek, isSameMonth } from 'date-fns';
...
export default function App() {
  ...
  const chartData = useMemo(() => {
    ...
    const sorted = [...measurements].sort((a, b) => parseISO(a.date).getTime() - parseISO(b.date).getTime());

    if (chartRange === 'daily') {
      return sorted.map(m => ({
        ...m,
        weight: unitSystem === 'metric' ? m.weight : kgToLbs(m.weight),
        displayDate: format(parseISO(m.date), 'd MMM', { locale: pl }),
        fullDate: format(parseISO(m.date), 'PPP', { locale: pl })
      }));
    }

    const groupedData: Record<string, { count: number; weight: number; bmr: number; tdee: number; date: Date }> = {};
    ...
    sorted.forEach(m => {
      ...
      if (!groupedData[key]) {
        groupedData[key] = { count: 0, weight: 0, bmr: 0, tdee: 0, date: new Date(key) };
      }

      groupedData[key].count++;
      groupedData[key].weight += m.weight;
      groupedData[key].bmr += m.bmr;
      groupedData[key].tdee += m.tdee;
    });

    return Object.values(groupedData).map(group => ({
      date: group.date.toISOString(),
      weight: Number((unitSystem === 'metric' ? (group.weight / group.count) : kgToLbs(group.weight / group.count)).toFixed(1)),
      bmr: Math.round(group.bmr / group.count),
      tdee: Math.round(group.tdee / group.count),
      displayDate: chartRange === 'weekly' 
        ? `${format(group.date, 'd MMM', { locale: pl })}`
        : format(group.date, 'MMM yyyy', { locale: pl }),
      fullDate: chartRange === 'weekly'
        ? `Tydzień od ${format(group.date, 'd MMMM', { locale: pl })}`
        : format(group.date, 'MMMM yyyy', { locale: pl })
    }));
  }, [measurements, chartRange, unitSystem]);

  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      const data = payload[0].payload;
      return (
        <div className="bg-white p-4 rounded-xl shadow-lg border border-[#141414]/5 text-sm">
          <p className="font-bold mb-2">{data.fullDate}</p>
          <div className="space-y-1">
            <p className="text-[#141414]">
              <span className="opacity-50">Waga:</span> <span className="font-mono font-bold">{data.weight} {unitSystem === 'metric' ? 'kg' : 'lbs'}</span>
            </p>
            <p className="text-blue-500">
              <span className="opacity-50">BMR:</span> <span className="font-mono font-bold">{data.bmr} kcal</span>
            </p>
            <p className="text-orange-500">
              <span className="opacity-50">TDEE:</span> <span className="font-mono font-bold">{data.tdee} kcal</span>
            </p>
          </div>
        </div>
      );
      ...
      <main className="flex-1 p-6 md:p-12 overflow-y-auto pb-24 md:pb-12">
        <AnimatePresence mode="wait">
          {activeTab === 'dashboard' && (
            ...
            >
              <header>
                <h2 className="text-3xl md:text-4xl font-serif italic">Witaj z powrotem</h2>
                  ...
                  <div className="flex items-center justify-between mb-8">
                    ...
                    <div className="flex bg-[#F5F5F0] rounded-lg p-1">
                      {(['daily', 'weekly', 'monthly'] as const).map((range) => (
                        ...
                        >
                          {range === 'daily' ? 'Dzień' : range === 'weekly' ? 'Tydzień' : 'Miesiąc'}
                        </button>
                      ))}
                    </div>
                  </div>
          ...
          {activeTab === 'calories' && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -20 }}
            >
              <CalorieTracker user={profile} token={token!} />
            </motion.div>
          )}
        </AnimatePresence>
